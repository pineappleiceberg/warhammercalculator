...
function reviewedTableGeometry(state, overrides = {}) {
  const objectivePositions = replayBattleState(state).mission.objectives.map(
    (objective, index) => ({
      ...
      deploymentZonePlayerId: index === 0 ? "player-1" : index === 4 ? "player-2" : "",
    }),
  );
  return {
...
test("migrates a version-2 roster battle with explicit untimed provenance", () => {
  ...
  assert.deepEqual(migrated.migration, {
    ...
    legacyPrioritisedEfficiencyThroughSequence: 3,
  });
...
test("migrates a version-3 guided battle without reclassifying timed events", () => {
  ...
  assert.deepEqual(migrated.migration, {
    ...
    legacyPrioritisedEfficiencyThroughSequence: 2,
  });
...
test("migrates version-40 battles without inventing detachment-rule selections", () => {
  const grimResolveOverrides = {
    ...exactMissionOverrides,
    players: {
  ...
  const current = initializeBattleForLists({
    ...
    ruleCoverageMatrix,
    missionPackCatalogue,
    ruleSelectionOverrides: grimResolveOverrides,
    id: "version-40-detachment-state",
  });
  const versionForty = normalizeBattleState({
    ...current,
    version: 40,
    migration: undefined,
  });
  ...
  assert.equal(migrated.version, BATTLE_STATE_VERSION);
  assert.equal(migrated.migration.sourceVersion, 40);
  assert.equal(
    migrated.migration.legacyDetachmentRulesThroughSequence,
    Math.max(...versionForty.events.map((event) => event.sequence)),
  );
  const replayed = replayBattleState(migrated);
...
test("migrates version-43 battles without inventing Shadow in the Warp activations", () => {
  const current = initializeBattleForLists({
    catalogue,
    firstList: attackers,
    secondList: defenders,
    ...
    missionPackCatalogue,
    ruleSelectionOverrides: exactMissionOverrides,
    id: "version-43-shadow-state",
  });
  const versionFortyThree = normalizeBattleState({
    ...current,
    version: 43,
  });
  const migrated = initializeBattleForLists({
    ...
    missionPackCatalogue,
    ruleSelectionOverrides: exactMissionOverrides,
    state: versionFortyThree,
    id: versionFortyThree.id,
  });
  assert.equal(migrated.version, BATTLE_STATE_VERSION);
  assert.equal(migrated.migration.sourceVersion, 43);
  assert.equal(
    migrated.migration.legacyShadowInTheWarpThroughSequence,
    Math.max(...versionFortyThree.events.map((event) => event.sequence)),
  );
  const replayed = replayBattleState(migrated);
  assert.equal(replayed.shadowInTheWarpActivations.length, 0);
  assert.equal(replayed.shadowInTheWarpResolutions.length, 0);
});

test("migrates version-44 battles behind the legacy Battle-shock comparator boundary", () => {
  const current = initializeBattleForLists({
    catalogue,
    firstList: attackers,
    secondList: defenders,
    ...
    missionPackCatalogue,
    ruleSelectionOverrides: exactMissionOverrides,
    id: "version-44-battle-shock-comparator-state",
  });
  const versionFortyFour = normalizeBattleState({
    ...current,
    version: 44,
  });
  const migrated = initializeBattleForLists({
    ...
    missionPackCatalogue,
    ruleSelectionOverrides: exactMissionOverrides,
    state: versionFortyFour,
    id: versionFortyFour.id,
  });
  assert.equal(migrated.version, BATTLE_STATE_VERSION);
  assert.equal(migrated.migration.sourceVersion, 44);
  assert.equal(
    migrated.migration.legacyBattleShockComparatorThroughSequence,
...
test("migrates version-46 battles without inventing Desperate Escape tests", () => {
  const current = initializeBattleForLists({
    catalogue,
    firstList: attackers,
    secondList: defenders,
    ...
    missionPackCatalogue,
    ruleSelectionOverrides: exactMissionOverrides,
    id: "version-46-desperate-escape-state",
  });
  const versionFortySix = normalizeBattleState({ ...current, version: 46 });
  const migrated = initializeBattleForLists({
    catalogue,
    firstList: attackers,
    secondList: defenders,
    ...
    missionPackCatalogue,
    ruleSelectionOverrides: exactMissionOverrides,
    state: versionFortySix,
    id: versionFortySix.id,
  });
  assert.equal(migrated.version, BATTLE_STATE_VERSION);
  assert.equal(migrated.migration.sourceVersion, 46);
  assert.equal(
    migrated.migration.legacyDesperateEscapeThroughSequence,
    Math.max(...versionFortySix.events.map((event) => event.sequence)),
  );
  const replayed = replayBattleState(migrated);
...
test("migrates version-47 battles without inventing Attached-unit separations", () => {
  const current = initializeBattleForLists({
    catalogue,
    firstList: attackers,
    secondList: defenders,
    ...
    missionPackCatalogue,
    ruleSelectionOverrides: exactMissionOverrides,
    id: "version-47-attached-separation-state",
  });
  const versionFortySeven = normalizeBattleState({ ...current, version: 47 });
  const migrated = initializeBattleForLists({
    catalogue,
    firstList: attackers,
    secondList: defenders,
    ...
    missionPackCatalogue,
    ruleSelectionOverrides: exactMissionOverrides,
    state: versionFortySeven,
    id: versionFortySeven.id,
  });
  assert.equal(migrated.version, BATTLE_STATE_VERSION);
  assert.equal(migrated.migration.sourceVersion, 47);
  assert.equal(
    migrated.migration.legacyAttachedSeparationThroughSequence,
    Math.max(...versionFortySeven.events.map((event) => event.sequence)),
  );
  const replayed = replayBattleState(migrated);
...
test("migrates version-49 battles without inventing Prioritised Efficiency transitions", () => {
  const current = initializeBattleForLists({
    catalogue,
    firstList: attackers,
    secondList: defenders,
    ...
    missionPackCatalogue,
    ruleSelectionOverrides: exactMissionOverrides,
    id: "version-49-prioritised-efficiency-state",
  });
  const versionFortyNine = normalizeBattleState({ ...current, version: 49 });
  const migrated = initializeBattleForLists({
    catalogue,
    firstList: attackers,
    secondList: defenders,
    ...
    missionPackCatalogue,
    ruleSelectionOverrides: exactMissionOverrides,
    state: versionFortyNine,
    id: versionFortyNine.id,
  });
  assert.equal(migrated.version, BATTLE_STATE_VERSION);
  assert.equal(migrated.migration.sourceVersion, 49);
  assert.equal(
    migrated.migration.legacyPrioritisedEfficiencyThroughSequence,
    Math.max(...versionFortyNine.events.map((event) => event.sequence)),
  );
  const replayed = replayBattleState(migrated);
  assert.equal(replayed.yieldPointAwards.length, 0);
  assert.equal(replayed.prioritisedEfficiencyToggles.length, 0);
  assert.equal(replayed.prioritisedEfficiencyModesByPlayer.size, 0);
});
