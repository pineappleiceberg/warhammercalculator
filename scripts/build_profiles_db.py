...
CREATE TABLE unit_transport_additional_pools (
    datasheet_id TEXT NOT NULL REFERENCES unit_transport(datasheet_id) ON DELETE CASCADE,
...
CREATE TABLE unit_transport_additional_pool_keywords (
    datasheet_id TEXT NOT NULL,
    ...
    FOREIGN KEY (datasheet_id, pool_position)
        REFERENCES unit_transport_additional_pools(datasheet_id, pool_position)
        ON DELETE CASCADE
) WITHOUT ROWID;
...
CREATE TABLE unit_transport_exclusion_exception_keywords (
    datasheet_id TEXT NOT NULL,
    group_position INTEGER NOT NULL,
    exception_group_position INTEGER NOT NULL CHECK (exception_group_position >= 1),
    keyword_position INTEGER NOT NULL CHECK (keyword_position >= 1),
    keyword TEXT NOT NULL,
    PRIMARY KEY (
        datasheet_id,
        group_position,
        exception_group_position,
        keyword_position
    ),
    FOREIGN KEY (datasheet_id, group_position)
        REFERENCES unit_transport_exclusion_groups(datasheet_id, group_position) ON DELETE CASCADE
...
def populate_transports(connection: sqlite3.Connection) -> tuple[int, int]:
    ...
    for datasheet_id, source in connection.execute(
        ...
        for pool_position, pool in enumerate(rules["additionalPools"], start=1):
            connection.execute(
                """INSERT INTO unit_transport_additional_pools
                   (datasheet_id, pool_position, capacity) VALUES (?, ?, ?)""",
                (datasheet_id, pool_position, pool["capacity"]),
            )
            for group_position, keywords in enumerate(pool["allowed"], start=1):
                connection.executemany(
                    """INSERT INTO unit_transport_additional_pool_keywords
                       (datasheet_id, pool_position, group_position,
                        keyword_position, keyword) VALUES (?, ?, ?, ?, ?)""",
                    (
                        (
                            datasheet_id,
                            pool_position,
        ...
        for group_position, group in enumerate(rules["excluded"], start=1):
            ...
            for exception_group_position, keywords in enumerate(
                group.get("keywordExceptions", []), start=1
            ):
                connection.executemany(
                    """INSERT INTO unit_transport_exclusion_exception_keywords
                       (datasheet_id, group_position, exception_group_position,
                        keyword_position, keyword)
                       VALUES (?, ?, ?, ?, ?)""",
                    (
                        (
