...
async function loadCatalogue(request: Request, env: Env) {
  cataloguePromise ??= env.ASSETS
    .fetch(new Request(new URL("/profile-data.json", request.url)))
    .then(async (response) => {
    ...
    const imports = {
      env: {
        ...
        emscripten_memcpy_big(destination: number, source: number, count: number) {
          ...
          new Uint8Array(calculator.memory.buffer).copyWithin(
            destination,
            source,
            source + count,
          );
...
async function handleApi(request: Request, env: Env) {
  ...
  try {
    ...
    if (url.pathname === "/api/v1/profiles" && request.method === "GET") {
      return json(await loadCatalogue(request, env), 200, { "Cache-Control": "public, max-age=3600" });
    }
    ...
    if ((url.pathname === "/api/v1/weapons" || url.pathname === "/api/v1/targets")
      && request.method === "GET") {
    ...
    if (url.pathname === "/api/v1/calculate" && request.method === "POST") {
      ...
      return json({ data: await exactCalculation(profile, request, env), profile, apiVersion: "v1" });
    }
