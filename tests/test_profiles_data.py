...
from scripts.build_profiles_db import (
    ...
    single_model_defensive_equipment_ability,
)
...
class ProfileDataTests(unittest.TestCase):
    ...
    def test_starting_size_parser_preserves_discrete_alternatives(self):
        ...
        self.assertEqual(
            duplicate_choice_allowance(
                "Any number of models can each be equipped with up to three of the "
        ...
        self.assertEqual(
            different_choice_allowance(
                "This model’s bolt pistol and boltgun can be replaced with two "
        ...
        self.assertEqual(
            pistol_pairing_rule(
                "* This model can only be equipped with two ranged weapons if one of "
                "them is a Pistol (and it can only have one Pistol)."
            ),
            ("Ranged", 2, "pistol", 1, 1),
        )
        self.assertTrue(
        ...
        self.assertEqual(
            simple_choice_item("1 shield generator*", {}),
            ("equipment:shield generator", "shield generator", 1),
        )
        self.assertEqual(
            simple_choice_item("Shining aegis", {}),
            ("equipment:shining aegis", "Shining aegis", 1),
        )
        self.assertTrue(has_single_footnote_marker("1 shield generator*"))
        self.assertFalse(has_single_footnote_marker("duplicates**"))
        self.assertEqual(
            weapon_type_limit(
        ...
        with closing(sqlite3.connect(":memory:")) as connection:
            connection.execute(
                """CREATE TABLE wargear_choice_alternatives (
        ...
        self.assertEqual(
            choice_weapon_vector("Lastrum storm bolter", known, set()),
            {"unit:1": ("Lastrum storm bolter", 1)},
        )
        self.assertEqual(
            default_loadout_clauses(
    ...
    def test_single_model_defensive_presets_follow_structured_equipment_choices(self):
        ...
        try:
            ...
            self.assertEqual(
                connection.execute(
                    """SELECT datasheets.name, presets.name,
                              presets.source_equipment_choice_exact,
                              count(links.alternative_position)
                       FROM unit_combat_presets AS presets
                       JOIN datasheets ON datasheets.id = presets.datasheet_id
                       JOIN unit_combat_preset_wargear_alternatives AS links USING
                           (datasheet_id, ability_position, preset_position)
                       WHERE (datasheets.name GLOB '*Keeper*'
                           OR datasheets.name = 'Lokhust Lord'
                           OR datasheets.name = 'Wazbom Blastajet'
                           OR datasheets.name GLOB 'Wolf Guard Pack Leader*')
                         AND presets.name IN
                           ('Shining Aegis', 'Nanoscarab Amulet',
                            'Blastajet Force Field', 'Storm Shield')
                       GROUP BY datasheets.name, presets.name,
                                presets.source_equipment_choice_exact
                       ORDER BY datasheets.name"""
                ).fetchall(),
                [
                    ("Keeper Of Secrets", "Shining Aegis", 1, 3),
                    ("Keeper of Secrets", "Shining Aegis", 1, 3),
                    ("Lokhust Lord", "Nanoscarab Amulet", 1, 2),
                    ("Wazbom Blastajet", "Blastajet Force Field", 1, 1),
                    ("Wolf Guard Pack Leader", "Storm Shield", 1, 10),
                    ("Wolf Guard Pack Leader With Jump Pack", "Storm Shield", 1, 9),
                ],
            )
            self.assertEqual(
                connection.execute(
            ...
            self.assertEqual(
                connection.execute(
                    """SELECT option_position, alternative_position, quantity_delta
                       FROM unit_combat_preset_wargear_alternatives
                       WHERE datasheet_id = '000000402'
    ...
    def test_battlesuit_duplicate_pools_share_item_and_weapon_type_limits(self):
        connection = sqlite3.connect(DATABASE)
        try:
            self.assertEqual(
                connection.execute(
                    """SELECT datasheet_id, option_position, fixed_limit,
                              limit_per_increment, models_per_increment
                       FROM wargear_choice_pools
                       WHERE datasheet_id IN
                           ('000000402', '000000418', '000001477', '000001478')
            ...
            self.assertEqual(
                connection.execute(
                    """SELECT datasheet_id, fixed_limit, limit_per_increment,
                              models_per_increment
                       FROM wargear_choice_item_limits
                       WHERE item_key = 'equipment:shield generator'
                       ORDER BY datasheet_id"""
                ).fetchall(),
                [
                    ("000000402", 1, 0, 1),
            ...
            self.assertEqual(
                connection.execute(
                    """SELECT weapon_type, fixed_limit, limit_per_increment,
                              models_per_increment
                       FROM wargear_weapon_type_limits
                       WHERE datasheet_id = '000000418'"""
                ).fetchone(),
                ("Ranged", 0, 3, 1),
            )
            self.assertEqual(
                connection.execute(
                    """SELECT option_position, alternative_position,
                              selection_key, selection_quantity
                       FROM wargear_choice_alternatives
                       WHERE datasheet_id = '000000418'
                         AND selection_key = 'equipment:shield generator'
                       ORDER BY option_position"""
                ).fetchall(),
                [
                    (1, 7, "equipment:shield generator", 1),
                    (3, 8, "equipment:shield generator", 1),
                ],
            )
            self.assertEqual(
                connection.execute(
                    """SELECT choice_coverage_exact
                       FROM unit_defensive_equipment_options
                       WHERE datasheet_id = '000000418'
                         AND name = 'Shield Generator'"""
                ).fetchone(),
            ...
            plan = connection.execute(
                """EXPLAIN QUERY PLAN
                   SELECT item_key FROM wargear_choice_item_limits
                   WHERE datasheet_id = ? ORDER BY item_key""",
                ("000000418",),
            ).fetchall()
            self.assertTrue(any("USING PRIMARY KEY" in row[3] for row in plan))
        finally:
            connection.close()

    def test_distinct_weapon_pools_preserve_pistol_pairing_rules(self):
        connection = sqlite3.connect(DATABASE)
        try:
            self.assertEqual(
                connection.execute(
            ...
            self.assertEqual(
                connection.execute(
                    """SELECT fixed_limit, count(*)
                       FROM wargear_choice_pools
                       WHERE datasheet_id IN ('000002202', '000002802', '000002804')
                         AND option_position = 2
                       GROUP BY fixed_limit"""
                ).fetchall(),
                [(2, 3)],
            )
            self.assertEqual(
    ...
    def test_browser_catalogue_exposes_editable_necron_loadout_guidance(self):
        ...
        crisis = next(unit for unit in catalogue["units"] if unit["id"] == "000000418")
        self.assertEqual(
            [
                (pool["id"], pool["fixed"], pool["perIncrement"])
                for pool in crisis["wargearChoicePools"]
            ],
            [("000000418:1", 0, 1), ("000000418:3", 0, 3)],
        )
        self.assertEqual(
            [
                alternative["id"]
                ...
                if alternative.get("selectionKey") == "equipment:shield generator"
            ],
        ...
        self.assertEqual(
            next(
                limit
                for limit in crisis["wargearChoiceItemLimits"]
                if limit["itemKey"] == "equipment:shield generator"
            )["perIncrement"],
            1,
        )
        self.assertEqual(crisis["weaponTypeLimits"][0]["weaponType"], "Ranged")
        self.assertEqual(crisis["weaponTypeLimits"][0]["perIncrement"], 3)
        crisis_shield = next(
            option
            for option in crisis["defensiveEquipment"]
            if option["name"] == "Shield Generator"
        )
        self.assertTrue(crisis_shield["choiceCoverageExact"])
        self.assertEqual(
            [
                link["alternativeId"]
                for link in crisis_shield["choiceLinks"]
                if link["quantityDelta"] == 1
            ],
            ["000000418:1:7", "000000418:3:8"],
        )
            ...
            for pool in unit["wargearChoicePools"]:
                ...
                for alternative in pool["alternatives"]:
                    ...
                    for weapon in alternative.get("replaces", []):
                        ...
                        self.assertGreater(weapon["quantity"], 0)
            selection_keys = {
                alternative["selectionKey"]
                for pool in unit["wargearChoicePools"]
                for alternative in pool["alternatives"]
                if "selectionKey" in alternative
            }
            for limit in unit["wargearChoiceItemLimits"]:
                self.assertIn(limit["itemKey"], selection_keys)
                self.assertIn(limit["source"], unit["wargearOptions"])
            for limit in unit["weaponTypeLimits"]:
                self.assertIn(limit["weaponType"], {"Ranged", "Melee"})
                self.assertIn(limit["source"], unit["wargearOptions"])
    ...
    def test_browser_catalogue_preserves_source_leadership(self):
        catalogue = json.loads(CATALOGUE.read_text(encoding="utf-8"))
        actual = {
            model["id"]: model["leadership"]
            for unit in catalogue["units"]
            for model in unit["models"]
        }
        with closing(sqlite3.connect(DATABASE)) as connection:
            expected = dict(
                connection.execute(
                    """SELECT id, leadership_target FROM model_profiles
                       WHERE is_catalogue_model = 1"""
                )
            )
        self.assertEqual(actual, expected)
    ...
    def test_browser_catalogue_preserves_faction_ability_identity(self):
        ...
        shadow_units = {
            unit["id"]
            for unit in catalogue["units"]
            if "000000707" in unit["factionAbilityIds"]
        }
        self.assertGreater(len(shadow_units), 0)
